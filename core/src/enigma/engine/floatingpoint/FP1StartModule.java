...
//	private ArrayList<String> instructionList = new ArrayList<String>();
	...
	private int showNumberToSolveIndex = -1;
...
//		instructions.add("The number 15 represents an exponent of 0!");
		...
		showNumberToSolveIndex = instructionList.size();
		instructionList.add("Below is the number we're converting to binary");
...
//			}
	...
	private void handleStateChange(int newInstructionPointer) {
		...
		else if(newInstructionPointer == showNumberToSolveIndex)
		{
			if(drawableValueToSolveReference != null)
			{
				drawableValueToSolveReference.setVisible(true);
			}
		}
	...
	public void draw(SpriteBatch batch, float lastModuleFraction) {
		...
		if (lastComponentDone()) {
			if(drawInstruction && getFractionDone() < 1.0f){
