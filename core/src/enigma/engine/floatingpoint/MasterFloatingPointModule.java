...
public class MasterFloatingPointModule extends CourseModule
{
	...
	private DrawableString numberToSolve = new DrawableString("");
	...
	public MasterFloatingPointModule(OrthographicCamera camera)
	{
		...
		numberToSolve.setVisible(false);

		pickNumberToSolve();
		updateChildrenModules();
	}

	private void updateChildrenModules()
	{
	...
	private void pickNumberToSolve()
	{
		...
		boolean showNumberLessOne = random.nextInt() % 3 == 0; // 1/3 change for number to be less
																// than 0
		...
		else
		{
			...
			denominator = random.nextInt(250) + 1;
		}
